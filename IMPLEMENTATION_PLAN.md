...
**Статус: реализовано (ядро + CLI).**

| Цель | Результат |
|------|-----------|
| Режим нативного boot | ``Memory.boot()`` по умолчанию **не** вызывает ``_sync_from_opencode``; ``boot_context(..., sync_opencode=False)`` по умолчанию; ``run_cli_chat_boot`` без OC. Алиас ``boot_native_context``. |
| Легаси | ``EIDOS_SYNC_OPENCODE=1`` — прежнее поведение (sync при boot и в ``WorkingMemory.add_event``). |
| Импорт сессии | ``eidos.py import-opencode`` — последняя сессия (rich JSON + дедуп по ``source_id``); флаги ``--all``, ``--max-sessions``. |
| Совместимость | ``eidos boot --sync-opencode`` — одноразово подтянуть OC при выводе boot |

**Критерий готовности:** чистый каталог данных + нативный boot не требует ``opencode.db``; после ``import-opencode`` история видна в episodic.
