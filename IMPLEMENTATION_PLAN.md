...
## Фаза 9 — Несколько именованных пайплайнов + этика / pulse / sleep в CLI

| Цель | Результат |
|------|-----------|
| Режимы | Например `--pipeline chat|dev|research` с разными весами вызовов памяти |
| Этика / pulse | Явные вызовы после хода (или каждые K ходов), без дублирования неявной логики |
| Sleep | Вызов `Memory.sleep()` по порогу и `/sleep`; согласование с `AUTO_SLEEP_THRESHOLD` |

**Критерий готовности:** включение pulse не меняет формат основного ответа по умолчанию; sleep можно прогнать из CLI без Streamlit.
...
| Стриминг токенов | Опциональный флаг `--stream` в `chat` |
| Параллельный сбор контекста | Независимые запросы к episodic/external в `asyncio.gather` с лимитами |
...
Зависят от стабильного **Фаз 5–9** и явных границ «что можно отправить наружу».
