...
class Verifier:
    ...
    def verify(self, draft: str, query: str) -> dict[str, Any]:
            ...
            for ep in episodes:
                ...
                if len(overlap) >= 3:
                    if not any(claim[:40] in c for c in contradictions):
                        contradictions.append(f"эпизод [{ep.get('id','?')}]: {ep.get('summary','')[:80]}")
        ...
        if needs_correction:
            for iss in issues:
                corrections.append(f"Утверждение не подтверждено принципами: «{iss['claim']}»")
    ...
    @staticmethod
    def _correct(draft: str, result: dict) -> str:
