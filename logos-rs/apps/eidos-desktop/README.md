...
- Кнопка **agents** — правка `agents.yaml`: читается тот же файл, что у рантайма; сохранение в `data/config/agents.yaml` (или в путь из `EIDOS_AGENTS_CONFIG`, если он внутри repo/data). Hot-reload профиля нет — при необходимости перезапустите приложение.
...
**Следующие шаги (фаза 4+):** валидация YAML при сохранении; подхват профиля без перезапуска. Ранее: редактор только вне окна или `EIDOS_AGENTS_CONFIG`.
