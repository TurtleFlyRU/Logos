...
npm install
./run-dev.sh
# то же самое: npm run tauri dev
```

Заглушка без LLM: `EIDOS_DESKTOP_STUB=1 ./run-dev.sh`

### Как понять, что подхватился новый UI

В шапке окна (рядом с профилем) должна быть метка **`send-ui-20250520f`** (или новее). Если её нет — вы смотрите **старый** интерфейс.

### Почему «ноль изменений» в чате / спиннере

| Как запускаете | Что происходит |
|----------------|----------------|
| **`npm run tauri dev`** или **`./run-dev.sh`** | UI с Vite (`http://localhost:1420`) — **нужный способ** для разработки |
| **`cargo run -p eidos-desktop`** без пересборки | В бинарник **вшит** фронт на момент последнего `cargo build`; правки в `src/` не видны |
| Терминал **`eidos chat`** | Это **CLI**, не окно Tauri — другой интерфейс |
| Старое окно не закрыли | Закройте все «Эйдос» и запустите снова через `./run-dev.sh` |

Если привыкли к `cargo run`:
...
(сначала `npm run build`, потом `cargo build`, потом бинарник)
...
- Пайплайны в поле ввода: `/pipeline research …`, `/run`, `/review`; команда **`/options`** открывает боковую панель локальных опций (вкл/выкл подсказок при вводе **`/`**, краткая сводка `get_settings`, ссылки на полные «Настройки» и `agents.yaml`). Подсказки slash фильтруют список команд; **Enter** / **Tab** подставляют выбранную строку, стрелки — навигация, **Esc** — закрыть список.
- Кнопки и slash: **Env**, **Контекст** (визуальный бюджет + **тренд сессии** по замерам в `cli_sessions`; сводка, шкала, слои, чипы; учитывается черновик в поле ввода), **Пайплайны**, **Boot**, **Сон**, **Настройки**, **Новая сессия**
...
**Фаза 4+ (desktop):** подсветка YAML в редакторе agents (CodeMirror); тренды бюджета по сессии (история в `cli_sessions/*.json`, график в панели «Контекст»).
...
| Нет метки UI / старый чат | Не `cargo run` без rebuild; использовать `./run-dev.sh` |
